...
class Carousel {
  ...
  constructor() {
    this.imageDescriptionElement = carouselElement.querySelector(
  ...
  protected setImageDescription(currentImageElement: HTMLElement) {
    this.imageDescriptionElement.innerHTML =
      currentImageElement.getAttribute("data-description") || "";
  }
}
...
class SwipeCarousel extends Carousel {
  ...
  constructor(animationRatio: number, deltaTreshold: number) {
    super();
    this.buttonElement = carouselElement.querySelector(
      "[data-carousel-rewind]",
    ) as HTMLElement;
  ...
  private resetImage() {
    this.isDragged = false;
    this.currentImageElement.style.transitionDuration = `${
      0.3 * this.animationRatio
    }s`;
    this.currentImageElement.style.transform = "translate(-50%, -50%)";

    this.currentImageElement.removeEventListener(
      "mousemove",
      (event: MouseEvent) => this.moveImg(event),
    );
    this.currentImageElement.removeEventListener(
      "touchmove",
      (event: TouchEvent) => this.moveImg(event),
    );
    this.currentImageElement.removeEventListener(
      "mouseup",
      () => this.resetImage(),
  ...
  private reloadDefault() {
    document.querySelectorAll(imagesQuerySelector).forEach((elementNode, i) => {
      ...
      setTimeout(
        ...
        300 * i * this.animationRatio,
      );
    });

    this.currentImage =
      carouselElement.querySelectorAll(imagesQuerySelector).length;
    this.currentImageElement = getImageElement(this.currentImage);
    this.animateImages();
    this.setImgEvents();
    this.setImageDescription(this.currentImageElement);
  ...
  private selectImg(event: InteractionEvent) {
    ...
    this.currentImageElement.addEventListener(
      "mouseup",
      () => this.resetImage(),
    );
    this.currentImageElement.addEventListener(
  ...
  private moveImg(event: InteractionEvent) {
    ...
    this.currentImageElement.style.transitionDuration = `${
      0.1 * this.animationRatio
    }s`;
    this.currentImageElement.style.transform =
      `translate(-50%, -50%) translate3d(${newX}px, ${newY}px, 0)`;
  }
  ...
  private animateImages(
    ...
    carouselElement.querySelectorAll(imagesQuerySelector).forEach((element) => {
      ...
      imgElement.style.transform =
        `translate(-50%, -50%) translate(${randX}px, ${randY}px) rotate(${randDeg}deg)`;
      setTimeout(
        () => (imgElement.style.transitionDuration = `${
          0.3 * this.animationRatio
        }s`),
  ...
  public destructor(): void {
    carouselElement.querySelectorAll(imagesQuerySelector).forEach(
      (elementNode) => {
        const element = elementNode as HTMLElement;
  ...
  private setImgEvents() {
    this.currentImageElement.addEventListener(
      "mousedown",
      (event: InteractionEvent) => this.selectImg(event),
    );
    this.currentImageElement.addEventListener(
      "touchstart",
      (event: InteractionEvent) => this.selectImg(event),
...
class HorizontalCarousel extends Carousel {
  ...
  private calculateZIndex() {
    ...
    carouselElement.querySelectorAll(imagesQuerySelector).forEach(
      (imgElement: Element) => {
        const imgElementId = parseInt(
          imgElement.getAttribute("data-carousel-image") || "0",
          10,
        );
